...
public class MainActivity extends Activity {
	...
	private Button mTopLeft, mTopCenter, mTopRight, mMiddleLeft, mMiddleCenter,
			mMiddleRight, mBottomLeft, mBottomCenter, mBottomRight;
		...
		public void handleMessage(Message msg) {
			...
			case CONNECTED_TO_SERVER:
				if (!a.mConnectedToServer) {
					...
					Toast.makeText(a,
							"Connected to server.  Finding opponent.",
	...
	private void clientTurn(int row, int col, View v) {
		if (!mIsMyTurn) {
			Toast.makeText(this, "It is not your turn.", Toast.LENGTH_LONG)
					.show();
		} else {
			turn(row, col, v);
		}
	...
	private void turn(int row, int col, View v) {
		if (!mGameStarted) {
			Toast.makeText(this, "The game has not started yet.",
					Toast.LENGTH_LONG).show();
			return;
		} else if (mTicTacToe.isGameOver()) {
			Toast.makeText(this, "The game has already ended.",
					Toast.LENGTH_LONG).show();
			return;
		}
	...
	public void onGameStartClick(View v) {
		...
		if (!mConnectRequested && mBound) {
			...
			Toast.makeText(
					this,
					"Already connected to server.  Currently finding opponent.",
					Toast.LENGTH_LONG).show();
		}
	}
